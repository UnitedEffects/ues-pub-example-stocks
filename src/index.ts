...
import iexConfig from "./iex-config";
...
async function getStockPrices(nc: any, symbol: string): Promise<void> {
    try {
        ...
        console.log(`Latest price for ${companyName} (${symbol}): $${stockPrice}`);
        const envelope = {
            created,
            type: 'stock-data',
            data: {
        ...
        await js.publish(`${uestreamConfig.SUBJECT}.${symbol}`, sc.encode(JSON.stringify(envelope)));
    } catch (error: any) {
...
async function doThis(nc: any) {
    setInterval(async () => {
        await getStockPrices(nc, 'AMZN');
        await getStockPrices(nc, 'SPY');
        await getStockPrices(nc, 'AAPL');
        await getStockPrices(nc, 'USO');
    }, 30*1000);
}
